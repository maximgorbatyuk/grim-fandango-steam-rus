...
# из уже склонированного репозитория.
#
# Файлы берутся из каталогов рядом с этим скриптом. Если репозитория на диске
# нет, используйте install.sh — он скачает всё сам и вызовет этот скрипт.
#
# Что делает:
...
usage() {
	cat <<-EOF
		Установка русификации Grim Fandango Remastered из этого репозитория.

		  ./local-install.sh [путь-к-GrimFandango.app]
		  ./local-install.sh --dry-run     показать план, ничего не менять
		  ./local-install.sh --help

		Без аргументов игра ищется по стандартному пути библиотеки Steam:
		  $DEFAULT_APP

		Нет репозитория на диске? Используйте install.sh — он скачает файлы.
	EOF
}
